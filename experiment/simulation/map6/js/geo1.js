...
$("#select8-1").on("change", function () {
  $(this).next("span").remove();
  m = $(this).val();
  if (solidline == 0 && m == 200) $(this).prop("disabled", true);
  else if (solidline == 1 && m == 300) $(this).prop("disabled", true);
  else if (solidline == 2 && m == 300) $(this).prop("disabled", true);
  else if (solidline == 3 && m == 400) $(this).prop("disabled", true);
  else if (solidline == 4 && m == 500) $(this).prop("disabled", true);
  else if (solidline == 5 && m == 300) $(this).prop("disabled", true);
...
$("#select8-2").on("change", function () {
  $(this).next("span").remove();
  n = $(this).val();
  if (solidline == 0 && n == 300) $(this).prop("disabled", true);
  else if (solidline == 1 && n == 400) $(this).prop("disabled", true);
  else if (solidline == 2 && n == 400) $(this).prop("disabled", true);
  else if (solidline == 3 && n == 500) $(this).prop("disabled", true);
  else if (solidline == 4 && n == 400) $(this).prop("disabled", true);
  else if (solidline == 5 && n == 200) $(this).prop("disabled", true);
...
function gameLoop() {
  ...
  if ((grid_size * ball.y) / 100 >= -40 && (grid_size * ball.y) / 100 <= -33) {
    ...
    ball.x += xunits * 1;
    ball.y += yunits * 1;
  } else if ((grid_size * ball.y) / 100 < 0) drawScreen();
}
...
$("#step16-1").click(function () {
  ...
  $("#resultContainer").html(
    '<img id="ctmap" style="position: absolute; left: 55px; top: 120px;" src="images/ctrmap1.png"/>'
  );
});
  ...
  if (simsubscreennum == 1) {
    ...
    setTimeout(function () {
      ...
      document.getElementById("can12-4").innerHTML =
        "&#10148;For T1>T2: Angle of Dip = tan<sup>-1</sup>⁡( (T1-T2)/d1)";
      document.getElementById("can12-5").innerHTML =
        "&#10148;For T2>T1: Angle of Dip =tan<sup>-1</sup>⁡( (T2-T1)/d1)";
      ctx2.beginPath();
      ctx.lineWidth = "5";
      ctx2.strokeStyle = "black";
...
function plotYaxis() {
  ...
  if (ypoints[ty].x != undefined) ctx.lineTo(ypoints[ty].x, ypoints[ty].y);
  ctx.stroke();
  ctx.closePath();
  ty++;
}
...
function tickYaxis() {
  ...
  for (i = 1; i < num_lines_y; i++) {
    ...
    ctx.textAlign = "start";
    ctx.fillText(
      xaxis_starting_point.number * i + xaxis_starting_point.suffix,
      -30,
      -grid_size * i
    );
    ctx.stroke();
...
function joinPoints() {
  ...
  if (jpts == 4) {
    ctx.quadraticCurveTo(
      ...
      -grid_size * (dotpoints[jpts][1] / 100 + 0.35),
      grid_size * (dotpoints[jpts][0] / 100),
      -grid_size * (dotpoints[jpts][1] / 100)
    );
  }
  if (
    jpts == 0 ||
...
function drawopattern() {
  ...
  generateQuestion(
    q1,
    "Identify the region with this pattern <span style = 'color:red'>+ + +</span> ",
    "",
    "Conglomerate",
    "Limestone",
    "Redstone",
  ...
  if (sY == dY) {
    ...
    if (vtCount == 6) {
      ...
      for (i = 0; i <= 1; i++) {
        ...
        if (i == 1) cell.innerHTML = "x";
      }
      document.getElementById("legend").style.visibility = "visible";
      var q2 = Object.create(questions);
      generateQuestion(
        q2,
        "Vertical Thickness of Redsandstone region is ______: ",
        "",
        "420m",
        "110m",
        "180m",
